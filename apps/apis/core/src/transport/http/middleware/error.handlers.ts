...
import { GmailSourceError } from "../../../integrations/gmail/gmail.config.js";
import { WebhookError } from "../webhooks/webhook.types.js";
...
export const apiErrorHandler: ErrorHandler<AppEnvironment> = (error, c) => {
  ...
  if (error instanceof HttpInputError || error instanceof AgentValidationError) {
    return c.json(errorBody(error.code, error.message, requestId), 400);
  }

  if (error instanceof AgentNotFoundError || error instanceof EscalationNotFoundError) {
  ...
  if (error instanceof EscalationConflictError) {
