...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .escalations { display: grid; gap: .6rem; margin-bottom: 1rem; }
    .escalation { display: flex; align-items: center; justify-content: space-between; gap: 1rem; border: 1px solid #6b3137; background: #24171a; border-radius: 10px; padding: .75rem .9rem; }
    .escalation.failed { border-color: #ff5d68; }
...
<body>
  ...
  <main><div class="escalations" id="escalations"></div><div class="board" id="board"></div></main>
  <dialog id="login">
  ...
  <script type="module">
    ...
    const escalationsNode = document.querySelector('#escalations');
    ...
    function renderThread(thread) {
      ...
      const reply = element('button', '', 'Ask agent');
      ack.addEventListener('click', () => act(thread, 'ack', ack));
      snooze.addEventListener('click', () => act(thread, 'snooze', snooze));
      reply.addEventListener('click', () => replyToThread(thread, reply));
      actions.append(ack, snooze, reply);
      card.append(meta, title, source, actions);
      return card;
    }

    function renderEscalations() {
      escalationsNode.replaceChildren();
      for (const escalation of [...escalations.values()].filter(
        (item) => item.status !== 'sent' && item.status !== 'dismissed',
      )) {
        const row = element('div', 'escalation ' + escalation.status);
        const copy = element(
          'div',
          '',
          'SMS ' + escalation.status + ' · ' + (escalation.event.summary || escalation.event.type),
        );
        const action = element('button', '', escalation.status === 'failed' ? 'Retry' : 'Dismiss');
        action.addEventListener('click', () => actOnEscalation(escalation, action));
        row.append(copy, action);
        escalationsNode.append(row);
      }
    }

    async function replyToThread(thread, button) {
      const message = prompt('What should the agent help with in this thread?');
      ...
      try {
        const response = await fetch('/threads/' + thread.id + '/replies', {
          ...
          body: JSON.stringify({ actor: credentials().actor, message }),
        });
    ...
    async function actOnEscalation(escalation, button) {
      button.disabled = true;
      const action = escalation.status === 'failed' ? 'retry' : 'dismiss';
      try {
        const response = await fetch('/escalations/' + escalation.id + '/' + action, {
          method: 'POST',
          headers: authHeaders({ 'Content-Type': 'application/json' }),
          body: JSON.stringify({ actor: credentials().actor, reason: 'Dashboard action' }),
        });
        if (!response.ok) throw new Error('Escalation action failed (' + response.status + ')');
        escalations.delete(escalation.id);
        renderEscalations();
      } catch (error) {
        alert(error.message);
        button.disabled = false;
    ...
    async function loadEscalations() {
      const response = await fetch('/escalations', { headers: authHeaders() });
      if (response.status === 401 || response.status === 503) throw new Error('auth');
      if (!response.ok) throw new Error('Could not load escalations');
      const payload = await response.json();
      escalations.clear();
      for (const escalation of payload.data) escalations.set(escalation.id, escalation);
      renderEscalations();
    }
    ...
    async function boot() {
      ...
      try {
        await Promise.all([loadThreads(), loadEscalations()]);
        void connectStream();
      } catch (error) {
