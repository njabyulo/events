...
import { routerService } from "../routing.dependencies.js";
...
export const routingEngine = new RouterEngine({
  ...
  eventsChannel: routerConfig.eventsChannel,
  pollIntervalMs: routerConfig.pollIntervalMs,
  reconnectDelayMs: routerConfig.reconnectDelayMs,
  drain: () => routerService.drain(),
});

export function startRoutingEngine(): void {
  if (routerConfig.enabled) routingEngine.start();
}
